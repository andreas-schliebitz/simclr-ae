...
from simclr_ae.models.simclr.dataset import SimCLRDataset
...
class SimCLRDataModule(L.LightningDataModule):
    ...
    def default_transforms(
        ...
        if self.standardize and mean is not None and std is not None:
            assert np.all((mean >= 0) & (mean <= 1)) and np.all(
                (std >= 0) & (std <= 1)
            ), "Mean and std values have to be in [0,1] as 'ToTensor()' automatically rescales to [0,1]."
            transformations.append(transforms.Normalize(mean=mean, std=std))
            print(f"Normalizing non-train data using mean={mean} and std={std} values.")
        if compose:
    ...
    def train_transforms(
        ...
        transformations = [
            ...
            transforms.RandomApply([color_jitter], p=self.color_jitter_prob),
            transforms.RandomGrayscale(p=self.grayscale_prob),
            transforms.RandomApply(
                [
                    transforms.GaussianBlur(
                        kernel_size=_round_to_odd(0.1 * self.img_size)
                    )
                ],
                p=0.5,
            ),
    ...
    def setup(self, stage: str) -> None:
        match stage:
            case "fit":
                ...
                self.train_split = SimCLRDataset(
                    train_split,
                    transform=self.train_transforms(mean=train_mean, std=train_std),
                )
            ...
            case "predict":
                ...
                self.predict_split = SimCLRDataset(
                    predict_split,
                    transform=self.default_transforms(
